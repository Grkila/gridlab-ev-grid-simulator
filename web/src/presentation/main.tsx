...
const viewNames:Record<string,string>={experiments:'Experiments',results:'Results',network:'Network',strategies:'Strategies',benchmarks:'Benchmarks',rl:'Training',overview:'Overview'};
...
function Presentation(){
  ...
  return <div className={`presentation ${reduced?'reduced-motion':''} ${overview?'in-overview':''}`}>
    ...
    <header className="deck-header"><a className="deck-brand" href="/" title="Open GridLab"><span className="brand-symbol">G<span>↗</span></span>GridLab<span className="brand-divider"/>EV charging research</a><span className="chapter-label">{slide.chapter}</span><button onClick={()=>setSettings(v=>!v)} aria-label="Presentation settings" aria-expanded={settings}>⚙ Settings</button></header>
    <div className="reveal" ref={element}><div className="slides">
      {slides.map((s,i)=><section key={s.id} id={s.id} className={`story-slide kind-${s.kind}`} data-auto-animate={s.kind==='app'?'':undefined}>
        <div className="slide-copy"><p className="slide-eyebrow">{s.question||s.label||s.chapter}</p><h1>{s.title}{s.accent&&<><br/><em>{s.accent}</em></>}</h1><p className="slide-description">{s.body}</p>{s.id==='opening'&&<div className="opening-details"><span className="challenge-label">EV DAYS CHALLENGE</span><p className="team-title">Team Jokić &amp; Grković<sup>3</sup></p><p className="team-members">Nikola Jokić · Mina Grković · Selena Grković · Dušan Grković</p><div className="schneider-logo"><img src="/presentation/schneider-electric-logo.png" alt="Schneider Electric"/></div></div>}</div>
        ...
        {['hero','city','closing'].includes(s.kind)&&<div className="map-caption"><i/> NOVI SAD / 45.2671° N, 19.8335° E<small>Synthetic points · inferred lines · symbolic heights</small></div>}
    ...
    <div className={`app-stage ${appVisible?'visible':''} ${interactive?'interactive':''} ${slide.id==='chat-demo'?'with-chat-examples':''}`} aria-hidden={!appVisible} inert={!appVisible||undefined}>
      <div className="app-chrome"><span><i className={connection==='online'?'online':''}/>{connection==='online'?'LIVE APPLICATION':'APPLICATION VIEW'}<span className="app-path"> / {viewNames[slide.cue?.view || 'experiments']}{slide.cue?.step!==undefined?` / ${['Scenario','Vehicles','Strategies','Review'][slide.cue.step]}`:''}</span></span><div><span className="app-sync">{ack===slide.cue?.view?'View synchronized':''}</span><button onClick={()=>setInteractive(v=>!v)}>{interactive?'Lock interaction':'Use application'}</button><a href={`/?view=${slide.cue?.view||'experiments'}${runId?`&run=${encodeURIComponent(runId)}`:''}`} target="_blank" rel="noreferrer">Open ↗</a></div></div>
      {slide.id==='chat-demo'&&<div className="chat-examples"><span>Example prompt:</span>{[
        ['Explain the experiment',`Explain the settings and results of experiment ${runId||'selected from the catalog'}. State its limitations and what the result does not establish. Do not start a new experiment.`],
        ['Propose an algorithm','Propose a charging algorithm using departure times and remaining energy. Explain observations, constraints, and fallback behavior. Prepare a proposal without implementation or execution.'],
        ['Prepare a comparison','Plan a comparison of immediate and managed charging with identical vehicles, seeds, network, and energy requests. List the metrics. Do not start experiments.']
      ].map(([label,prompt])=><button key={label} onClick={()=>{setInteractive(true);iframe.current?.contentWindow?.postMessage({type:cueEvent,cue:{view:'strategies',section:'development',chat:true,chatPrompt:prompt,runId:runId||undefined}},location.origin);}}>{label}</button>)}<small>The prompt fills the composer. You choose when to send it.</small></div>}
      <iframe ref={iframe} title="GridLab interactive application" src="/?present=1&view=experiments" onLoad={sendCue} tabIndex={interactive?0:-1} inert={!interactive||undefined}/>
      {connection!=='online'&&<div className="app-unavailable"><span className="status-ring"/><h2>{connection==='checking'?'Connecting to GridLab…':'Application server disconnected'}</h2><p>Slides and study charts remain available. Start the local server to use the application.</p><code>python scripts/run_playground_app.py --port 8517</code><button onClick={()=>{void refresh();iframe.current?.contentWindow?.location.reload();}}>Reconnect</button></div>}
      {connection==='online'&&slide.cue?.view==='results'&&!runId&&<div className="run-prompt"><b>Select a completed local run</b><span>Create a run in Experiments to populate this view. Later charts show a separate completed study.</span><button onClick={()=>setSettings(true)}>Select run</button></div>}
    </div>
    {settings&&<aside className="deck-panel settings-panel" aria-label="Presentation settings"><div className="panel-title"><h2>Presentation settings</h2><button onClick={()=>setSettings(false)} aria-label="Close settings">×</button></div><label>Run for result views<select value={runId} onChange={e=>{setRunId(e.target.value);const url=new URL(location.href);if(e.target.value)url.searchParams.set('run',e.target.value);else url.searchParams.delete('run');history.replaceState(null,'',url);}}><option value="">Select run</option>{runId&&!runs.some(r=>r.run_id===runId)&&<option value={runId}>{runId} (not in catalog)</option>}{runs.map(r=><option key={r.run_id} value={r.run_id}>{r.name||r.run_id} · {r.status}</option>)}</select></label><p>{connection==='online'?`${runs.length} saved runs.`:'Start the server to access local runs.'} Slide navigation does not start runs or send messages.</p><button onClick={refresh}>Refresh connection and runs</button><label className="check-label"><input type="checkbox" checked={reduced} onChange={e=>setReduced(e.target.checked)}/> Reduce motion</label><a href="/presentation/map.html" target="_blank" rel="noreferrer">Open detailed map ↗</a><p>Map tiles require internet access. The 3D scene uses local data.</p><div className="shortcuts">← → Slides · O Overview · N Notes · F Fullscreen · Esc Close</div></aside>}
    {notes&&<aside className="deck-panel notes-panel" aria-label="Speaker notes"><div className="panel-title"><h2>{index+1} / Speaker notes</h2><button onClick={()=>setNotes(false)} aria-label="Close notes">×</button></div><h3>{slide.title} {slide.accent}</h3><p>{slide.note}</p>{slide.source&&<a href={sourceUrl(slide.source)} target="_blank" rel="noreferrer">Source: {slide.source} ↗</a>}<p className="next-note">NEXT / {slides[Math.min(index+1,slides.length-1)].title}</p></aside>}
    <footer className="deck-footer"><div className="chapter-progress">{chapters.map(ch=><button key={ch} className={ch===slide.chapter?'active':''} onClick={()=>deck.current?.slide(slides.findIndex(s=>s.chapter===ch))} title={ch} aria-label={`Open: ${ch}`}><span/></button>)}</div><div className="deck-actions"><button onClick={()=>deck.current?.toggleOverview()} aria-label="Slide overview" title="Overview (O)">▦</button><button onClick={()=>setNotes(v=>!v)} aria-pressed={notes}>Notes</button><button onClick={fullscreen} title="Fullscreen (F)">⛶</button><button onClick={()=>deck.current?.prev()} disabled={index===0} aria-label="Previous slide">←</button><select aria-label="Select slide" value={index} onChange={e=>deck.current?.slide(Number(e.target.value))}>{slides.map((s,i)=><option value={i} key={s.id}>{String(i+1).padStart(2,'0')} / {slides.length} · {s.title}</option>)}</select><button onClick={()=>deck.current?.next()} disabled={index===slides.length-1} aria-label="Next slide">→</button></div></footer>
    <div className="attribution"><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">© OpenStreetMap contributors · ODbL</a><span>Planning model · vehicle movement is illustrative</span></div>
    <div className="sr-only" aria-live="polite">Slide {index+1}: {slide.title} {slide.accent}</div>
  </div>;
}
