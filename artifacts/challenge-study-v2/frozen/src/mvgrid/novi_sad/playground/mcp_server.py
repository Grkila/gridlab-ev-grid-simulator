...
from .continuous_campaign import CampaignConfig, ContinuousCampaignService
...
def _start_continuous_campaign(campaign_id, resume=False):
    """The app owns chat-started workers so they survive the CLI process."""
    broker = os.environ.get('EV_PLAYGROUND_BROKER_URL')
    if not broker:
        return ContinuousCampaignService().start(campaign_id, resume=resume)
    ...
    if (parsed.scheme != 'http' or parsed.hostname not in ('127.0.0.1', 'localhost')
        ...
        raise ValueError('Continuous training broker must be a loopback HTTP origin.')
    # Validate the ID before incorporating it into a URL.
    campaign = ContinuousCampaignService()
    campaign.folder(campaign_id)
    action = 'resume' if resume else 'start'
    body = json.dumps({'runtime_root': str(campaign.root)}).encode('utf-8')
    try:
        with urlopen(Request(broker.rstrip('/')+f'/api/continuous-rl/campaigns/{campaign_id}/{action}',
                     data=body, headers={'Content-Type':'application/json'}), timeout=30) as response:
            return json.load(response)
    except HTTPError as exc:
        raise ValueError(json.loads(exc.read()).get('error', 'Continuous training broker rejected request')) from exc


@server.tool(annotations=READ, structured_output=True)
@tool_errors
def ev_get_continuous_rl_catalog() -> dict[str, Any]:
    """Discover node-PPO settings and campaigns; historical binary policies remain separate."""
    return ContinuousCampaignService().catalog()


@server.tool(annotations=WRITE, structured_output=True)
@tool_errors
def ev_create_continuous_campaign(benchmark_job_id: str, config: CampaignConfig) -> dict[str, Any]:
    """Freeze a bounded continuous PPO campaign from compatible benchmark anchor evidence; does not start training."""
    return ContinuousCampaignService().create(benchmark_job_id, config)


@server.tool(annotations=WRITE, structured_output=True)
@tool_errors
def ev_start_continuous_campaign(campaign_id: str) -> dict[str, Any]:
    """Start a prepared campaign under the shared worker lock; return its ID promptly."""
    return _start_continuous_campaign(campaign_id)


@server.tool(annotations=READ, structured_output=True)
@tool_errors
def ev_get_continuous_campaign(campaign_id: str) -> dict[str, Any]:
    """Inspect campaign phase, budget and execution status; completion is not scientific acceptance."""
    return ContinuousCampaignService().get(campaign_id)


@server.tool(annotations=READ, structured_output=True)
@tool_errors
def ev_get_continuous_results(campaign_id: str) -> dict[str, Any]:
    """Read frozen campaign inputs and actual training/held-out evidence, including incomplete outcomes."""
    return ContinuousCampaignService().results(campaign_id)


@server.tool(annotations=WRITE, structured_output=True)
@tool_errors
def ev_cancel_continuous_campaign(campaign_id: str) -> dict[str, Any]:
    """Request campaign cancellation; only complete PPO updates remain resumable."""
    return ContinuousCampaignService().cancel(campaign_id)


@server.tool(annotations=WRITE, structured_output=True)
@tool_errors
def ev_resume_continuous_campaign(campaign_id: str) -> dict[str, Any]:
    """Resume matching interrupted campaign checkpoints within its original frozen budget and binding."""
    return _start_continuous_campaign(campaign_id, resume=True)
