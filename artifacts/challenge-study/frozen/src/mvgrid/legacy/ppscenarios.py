...
from . import mapping
from . import stages
from mvgrid.paths import LEGACY_NET_CACHE, LEGACY_NET_OUTPUT, ensure_runtime_directories
...
def run_scenarios(data):
    ...
    if LEGACY_NET_CACHE.exists():
        rerun = (ez.ynbox(**yn_d))
    else:
        rerun = True
    net0, el0 = pp.populate_net(data) if rerun else ut.data_un(LEGACY_NET_CACHE)
    ut.data_p((net0, el0), LEGACY_NET_CACHE)
    scenarios_d = {
    ...
    if scenarios is not None:
        ...
        ppp.to_json(net1, LEGACY_NET_OUTPUT)
