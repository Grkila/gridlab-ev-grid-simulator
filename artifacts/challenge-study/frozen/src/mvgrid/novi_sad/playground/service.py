...
class Service:
    ...
    def get_run(self, run_id: str) -> dict:
        if run_id.startswith('ppo-'):
            from .continuous_campaign import ContinuousCampaignService
            state = ContinuousCampaignService(self.root).get(run_id)
            # Existing worker owners use starting/running as their active states.
            return {**state, 'run_id': run_id, 'campaign_status': state['status'],
                    'status': 'starting' if state['status'] == 'queued' else state['status']}
    ...
    def cancel_run(self, run_id: str) -> dict:
        if run_id.startswith('ppo-'):
            from .continuous_campaign import ContinuousCampaignService
            return {**ContinuousCampaignService(self.root).cancel(run_id), 'run_id': run_id}
        state = self.get_run(run_id)
        if state["status"] in ("starting", "running"):
