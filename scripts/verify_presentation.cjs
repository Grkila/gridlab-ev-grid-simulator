...
const output=path.join(root,'artifacts/playground/evidence/presentation');
...
(async()=>{
  ...
  try{
    ...
    const count=await page.locator('section.story-slide').count();assert.equal(count,50);
    ...
    await page.getByRole('button',{name:'Podešavanja prezentacije',exact:true}).click();
    ...
    await page.getByRole('button',{name:'Zatvori podešavanja',exact:true}).click();
    for(let i=0;i<count;i++){
      await page.getByRole('combobox',{name:'Izaberi slajd'}).selectOption(String(i));
      ...
      if(await page.locator('.app-stage.visible').count()){
        await page.waitForFunction(()=>document.querySelector('.app-sync')?.textContent==='Prikaz je usklađen');
    ...
    await page.getByRole('combobox',{name:'Izaberi slajd'}).selectOption('27');
    ...
    await page.getByRole('combobox',{name:'Izaberi slajd'}).selectOption('38');
    ...
    await page.getByRole('combobox',{name:'Izaberi slajd'}).selectOption('7');
    ...
    await page.getByRole('button',{name:'Beleške',exact:true}).click();
    assert(await page.getByRole('complementary',{name:'Beleške za izlagača'}).isVisible());
    await page.getByRole('button',{name:'Zatvori beleške',exact:true}).click();
    await page.getByRole('button',{name:'Pregled slajdova',exact:true}).click();
    await page.waitForSelector('.reveal.overview');
    assert.equal(await page.locator('.app-stage.visible').count(),0);
    await page.getByRole('button',{name:'Pregled slajdova',exact:true}).click();
    await page.getByRole('button',{name:'Podešavanja prezentacije',exact:true}).click();
    await page.getByRole('checkbox',{name:'Smanji animacije'}).check();
    ...
    await page.getByRole('button',{name:'Zatvori beleške',exact:true}).click();
    await page.getByRole('combobox',{name:'Izaberi slajd'}).selectOption('0');
    ...
    await page.getByText('Server aplikacije nije povezan',{exact:true}).waitFor();
    await page.getByRole('button',{name:'Sledeći slajd',exact:true}).click();
    ...
    console.log('Presentation acceptance passed: 50 slides, live integration, no writes or browser errors.');
