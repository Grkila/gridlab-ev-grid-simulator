...
(async () => {
  ...
  await page.route('**/api/**', async route => {
    ...
    if (request.method() === 'POST') {
      ...
      if (pathname === '/api/benchmarks/suites') { const suite={suite_id:'suite-'+(suites.length+1),config:body.definition,ladder:[0,2],tests:[]}; suites.push(suite);return send(suite); }
    ...
    reads.push(pathname);
    if (pathname === '/api/benchmarks') return send({strategies:[{id:'immediate',label:'Immediate'}],jobs:[],suites,tests:[],districts:[]});
    if (pathname === '/api/rl/catalog') return send({models:[]});
  ...
  assert.equal(await page.getByRole('link', { name: 'Training', exact: true }).isVisible(), false);
  await page.getByText('Research tools', { exact: true }).click();
  assert.equal(await page.getByRole('link', { name: 'Training', exact: true }).isVisible(), true);
  await page.getByText('Research tools', { exact: true }).click();
  ...
  assert.equal(await page.getByLabel('Charging schedule',{exact:true}).inputValue(),'home_only');
  await page.getByLabel('Charging schedule',{exact:true}).selectOption('whole_day');
  await page.getByLabel('residential charging percent').press('End');
  assert.equal(await page.getByLabel('residential charging percent').inputValue(),'100');
  assert.equal(await page.getByLabel('workplace charging percent').inputValue(),'0');
  assert.equal(await page.getByLabel('public charging percent').inputValue(),'0');
  await page.getByLabel('residential charging percent').press('Home');
  assert.equal(await page.getByLabel('workplace charging percent').inputValue(),'50');
  assert.equal(await page.getByLabel('public charging percent').inputValue(),'50');
  await page.getByLabel('residential charging percent').press('ArrowRight');
  const shares=await page.getByRole('slider').evaluateAll(sliders=>sliders.map(s=>Number(s.value)));
  assert.equal(shares.reduce((a,b)=>a+b,0),100);
  await page.getByLabel('Charging schedule',{exact:true}).selectOption('home_only');
  await page.getByLabel('Charging schedule',{exact:true}).selectOption('whole_day');
  assert.equal(await page.getByLabel('residential charging percent').inputValue(),'1');
  await page.screenshot({path:path.join(root,'artifacts/playground/whole-day-study/experiment-ui.png'),fullPage:true});
  ...
  await page.screenshot({ path: path.join(root, 'artifacts/playground/whole-day-study/workflow-desktop.png'), fullPage: true });
  ...
  assert.equal(writes[0].body.definition.fleet.fleet_size, 321); assert.equal(writes[0].body.definition.fleet.charging_profile,'whole_day');
  ...
  await page.screenshot({ path: path.join(root, 'artifacts/playground/whole-day-study/workflow-mobile.png'), fullPage: true });
  ...
  await page.goto(url+'/?view=benchmarks');
  await page.getByLabel('Benchmark charging schedule').waitFor();
  assert.equal(await page.getByLabel('Benchmark charging schedule').inputValue(),'whole_day');
  ...
  assert.equal(writes.at(-1).body.definition.charging_profile,'whole_day');
  await page.screenshot({path:path.join(root,'artifacts/playground/whole-day-study/benchmark-ui.png'),fullPage:true});
  ...
  await page.getByRole('button',{name:'Freeze 10-test benchmark'}).click();
  await page.getByText('Charging schedule: Home only',{exact:true}).waitFor();
  assert.equal(writes.at(-1).body.definition.charging_profile,'home_only');
  await page.getByLabel('Saved benchmark',{exact:true}).selectOption('suite-1');
  await page.getByText('Charging schedule: Whole day',{exact:true}).waitFor();
  assert.deepEqual(errors, []);
  console.log('PASS: navigation, four setup stages, linked percentage sliders, schedule presets, preserved inputs, save/start ordering, run history and reload, comparison, failed-start recovery, save only, mobile layout, invalid JSON, no browser errors.');
