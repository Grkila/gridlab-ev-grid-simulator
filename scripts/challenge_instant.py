...
def estimate(f,step,weights,committed=None,lv=.5,margin=1.):
    ...
    known={b['id'] for b in s.read(s.FROZEN/'blocks.json')}
    if (set(weights)|set(committed))-known: raise ValueError('Unknown node in placement or committed charging')
    if any(not math.isfinite(v) or v<0 for v in committed.values()): raise ValueError('Committed charging must be finite and nonnegative')
    if isinstance(step,bool) or not isinstance(step,int) or not 0<=step<len(f['gross_kw']): raise ValueError('Invalid interval index')
    if not math.isfinite(lv) or not 0<=lv<=1 or not math.isfinite(margin) or margin<=0: raise ValueError('Invalid capacity assumption')
    started=time.perf_counter(); probes=[]
...
def main():
    ...
    invalid=[]; node=next(iter(s.placement()))
    for label,weights,committed in [('unknown_node',{'missing-node':1.},{}),('negative_commitment',s.placement(),{node:-1.}),('nan_commitment',s.placement(),{node:float('nan')}),('bad_weights',{node:.5},{})]:
        try: estimate(f,72,weights,committed); invalid.append(dict(case=label,rejected=False))
        except ValueError as error: invalid.append(dict(case=label,rejected=True,error=str(error)))
    s.write(s.OUT/'admission-invalid-inputs.json',invalid)
