...
def reward_formula(grid_penalty_normalization='none'):
    if grid_penalty_normalization not in ('none','episode_horizon'):
        raise ValueError('Use grid penalty normalization none or episode_horizon.')
    return REWARD_FORMULA.replace('-20*grid_severity',
        '-20*grid_severity/episode_horizon_steps' if grid_penalty_normalization == 'episode_horizon' else '-20*grid_severity')
...
class ContinuousEVEnv(gym.Env):
    ...
    def __init__(self, episode_factory, node_ids=None):
        ...
        self.grid_penalty_normalization = getattr(episode_factory,'grid_penalty_normalization','none')
        if self.grid_penalty_normalization not in ('none','episode_horizon'):
            raise ValueError('Use grid penalty normalization none or episode_horizon.')
    ...
    def reset(self, *, seed=None, options=None):
        ...
        reward_config = fixture.get('reward_config',{'grid_penalty_normalization':self.grid_penalty_normalization})
        if reward_config != {'grid_penalty_normalization':self.grid_penalty_normalization}:
            raise ValueError('Episode reward configuration differs from the policy factory.')
        ...
        self.grid_penalty_divisor = len(demand) if self.grid_penalty_normalization == 'episode_horizon' else 1
        self.sim = Simulator()
    ...
    def step(self, action, *, _teacher_actions=None):
        ...
        terms = {'delivery': delivery/self.energy_scale,
                 ...
                 'grid': -20.*severity/self.grid_penalty_divisor,
        ...
        diagnostics = {'raw_action': raw.tolist(), 'bounded_action': bounded.tolist(),
                       ...
                       'reward_terms': terms, 'grid_penalty_divisor':self.grid_penalty_divisor, 'grid_rescue_enabled': _teacher_actions is not None}
    ...
    def _metrics(self, complete):
        ...
        return {'complete': bool(complete), 'passed': bool(complete and not violations and not nonconverged and energy_ok),
                ...
                'reward_config': {'grid_penalty_normalization':self.grid_penalty_normalization,
                                  'grid_penalty_divisor':self.grid_penalty_divisor},
                'reward_formula': reward_formula(self.grid_penalty_normalization),
    ...
    def get_state(self):
        ...
        return copy.deepcopy({'version': ENV_VERSION, 'node_ids': self.node_ids,
            'grid_penalty_normalization':self.grid_penalty_normalization,
    ...
    def set_state(self, state):
        ...
        if state.get('grid_penalty_normalization','none') != self.grid_penalty_normalization:
            raise ValueError('Environment checkpoint reward configuration mismatch.')
        self.sim = Simulator()
        self.sim.__dict__.update(state['sim'])
        self.grid_penalty_divisor = len(self.sim.demand) if self.grid_penalty_normalization == 'episode_horizon' else 1
